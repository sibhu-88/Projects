...
int main(void)
{
    ...
    UART_INIT();        // Function Call to INIT UART
    WIFI_CONF();        // Function Call to send AT commands to ESP8266
    ...
    while (1)
    {
        ...
        {
            COMMAND = BUFFER;
            if (COMMAND == LEDON) // Logic to set LED ON or OFF depending upon the received value from ESP8266
            {
